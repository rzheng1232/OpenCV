...
video = cv.VideoCapture('Images/walkingsand.mp4')
prevFrame = np.zeros(video.read()[1].shape[:3], dtype='uint8')
prevprevFrame = np.zeros(video.read()[1].shape[:3], dtype='uint8')
...
while True:
    ...
    for framenumber in range(len(previousFrames)):
        if framenumber%4 == 0:
            ...
            assert blur.dtype == previousFrames[index].dtype
            if (index == 0):
                difference = cv.absdiff(blur, previousFrames[index])
            else:
                difference = cv.absdiff(previousFrames[index-1], previousFrames[index])
