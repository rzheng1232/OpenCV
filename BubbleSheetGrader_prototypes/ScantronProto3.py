...
img = cv.imread('Photos/newScan6.jpg')
cv.imshow('image', img)
...
#print(bRects)
#print("                                                                     ")
bRects = sorted(bRects, key=lambda x: x[0])
...
for c in bRects:
    ...
    if w/h > .01 and w/h < 5 and w > 25:
        Inc = Inc+1
        cx = (int)(x + w/2)
        cy = (int)(y + h/2)
        #print(cy-prevCenterY)
        if abs(cy - prevCenterY) > 2:
            redInc = redInc + 50
